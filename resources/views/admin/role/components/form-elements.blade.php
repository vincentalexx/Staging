<div class="form-group row align-items-center" :class="{'has-danger': errors.has('name'), 'has-success': this.fields.name && this.fields.name.valid }">
    <label for="name" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.role.columns.name') }} <span class="required-input">*</span></label>
...
<div class="form-group row align-items-center">
	<label for="name" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'" style="top: -18rem;">
        Permissions <span class="required-input">*</span>
    </label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <permissions
            v-model="form.permissions"
            permissions={{ json_encode($permissions) }}
            selected-permissions={{ json_encode($selectedPermissions) }}
        ></permissions>
	</div>
</div>